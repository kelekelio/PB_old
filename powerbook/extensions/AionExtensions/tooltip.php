...
$cachefile = 'cache/tooltip/'.$language.'/'.$id . $cl .'.html';
$cachetime = 604800000;
...
function skillenchant($name, $language)
{
	...
	return '<img src="https://aionpowerbook.com/skillicon/' . $skillicon . '.png" alt="" class="thumb3">  <a href="https://aionpowerbook.com/powerbook/Skill/' . $id . '" class="skilltipzy" skillid="' . $id . '" skilltiplang="' . $language . '">' . $localizationSkill . '</a>';
}
...
function client_pc_skill_skin($n, $t, $l)
{
	...
	$sql = "SELECT * FROM client_pc_skill_skin WHERE name = '$n' ";
	
		$query=mysqli_query($conn, $sql) or die(mysqli_error());
		while ($result = mysqli_fetch_assoc($query)) {
			$desc_long = translate($result["desc_long"], language());
		}	
...
";
    ...
    while ($itemrow=mysqli_fetch_assoc($query)) {
		$enchant_prob	= $itemrow["enchant_prob"]/10;
		$enchant_prob1	= $itemrow["enchant_prob1"]/10;
		$enchant_prob2	= $itemrow["enchant_prob2"]/10;
	...
	$enchantSkilltabname = explode('(', translate('STR_TOOLTIP_ITEM_SKILL_ENCHANT_TITLE_CAN_REIDENTIFY', language()));
	
$enchantSkill .= '<div class="wrap item_set" style=""><dl class="item_set"><dt class="item_set"><a href="#" class="enchantskills" onclick="return false;"><img src="https://aionpowerbook.com/pbimg/enchskill1.png" class="thumb3"> ' . $enchantSkilltabname[0] . '</a></dt><div class="enchantskillsdetails" style=""><br>';
		
$enchantSkill .= '+1: ' . $enchant_prob . '%  ';
	if ($enchant_prob1 >= 1) {
  $enchantSkill .= '|  +2: ' . $enchant_prob1 . '%  ';
	}
	if ($enchant_prob2 >= 1) {
	$enchantSkill .='|  +3: ' . $enchant_prob2 . '%';
	}
	$enchantSkill .='<br>';
...
";
	...
	return '<img src="https://aionpowerbook.com/item/icon'.$cl.'/' . $extraicon_name . '.png" alt="" class="thumb3">  <a class="col_' . $quality . ' bold" href="https://aionpowerbook.com/powerbook/Item/' . $result["id"] . '">' . $localizationExtra . '</a>';
...
while( $row=mysqli_fetch_array($query) ) {
		...
		$localization_desc2 = translate($row["desc_long2"], $language);
...
include 'include/leveldata.php';
		...
		if($grind_slot_category != NULL) {
			...
			$odianimg .= '<img src="'.$OdianRuneIcon.'" class="enchantingTip" title="'. $grind_slot_opening_cost .' <img style=\'width:15px\' src=\'https://aionpowerbook.com/item/icon/icon_item_mix_crystal_05.png\'> " > ';
...
include 'include/maptemplate.php';		
		...
		if ($random_option_name != NULL){
			$RealrandomStatsfinal = RealrandomStats($random_option_name , $language, 'tool');
		}
		if ($random_option_set != NULL && $random_option_name == NULL){
			$randomStatsfixed = '<div class="wrap weapon_property_bonus" style=""><p class="randombonuses"><img src="https://aionpowerbook.com/pbimg/random1.png" class="thumb3"> Random Bonuses</p></div>';
		}
...
$basicStats .= $newbasicstats;
	...
	elseif (($id >= 187300000) && ($id <= 187309999)) {
		...
		if ($odian_skill_name != NULL) {
			$odianDet .= '<div class="wrap item_set" style=""><dl class="item_set"><dt style="width:50px !important">' . translate('STR_PLAYER_INFO_DIALOG__SKILL', $language) . ':</dt><br>';
			$odianDet .= skinskill($odian_skill_name, $language);
			$odianDet .= '<br><br>' . translate($odian_skill_desc_1, $language);
...
echo '<div class="objects object' . $objectType . '"><div class="head item_basic ' . $css . '"><dl class="basic">';											//starting div
echo '<dd class="thumb"><img src="https://aionpowerbook.com/item/icon'.$cl.'/' . $itemicon . '.png" alt="">'.$itempvpve.'</dd>';					//icon, 3d icon and tooltip overlay if necessary
...
//echo '<div class="hbody"><div class="objects object' . $objectType . '"><div class="head item_basic"><dl class="basic">' . $localization . '</dd></dl></div></div></div>';
