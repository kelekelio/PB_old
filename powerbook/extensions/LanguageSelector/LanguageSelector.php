...
function wfLanguageSelectorGetLanguageCode( $user, &$code ) {
    ...
    //if URL has &aionclassic = 1, set AionClassic cookie to the value
...
function wfLanguageSelectorSkinTemplateOutputPageBeforeExec( &$skin, &$tpl ) {
    ...
    switch( $wgLanguageSelectorLocation ) {
        ...
        case LANGUAGE_SELECTOR_INTO_SITENOTICE: $key = 'headelement'; break;
...
function wfAionClassicCheckBox () {
    ...
    //radio ($name, $value, $checked=false, $attribs=array())
...
function wfLanguageSelectorHTML( Title $title, $style = null, $class = null, $selectorstyle = null, $buttonstyle = null, $showCode = null ) {
    ...
    if ($ll != NULL) {
        ...
        $html .= Html::Hidden( 'll', $ll );

    }

    if ($hl != NULL) {

        $html .= Html::Hidden( 'hl', $hl );

    }

    if ($region != NULL) {

        $html .= Html::Hidden( 'region', $region );

    }

    if ($race != NULL) {

        $html .= Html::Hidden( 'race', $race );
    ...
    foreach ( wfGetLanguageSelectorLanguages() as $ln ) {
        ...
        $html .= Xml::option( $name, $ln, $ln == $code, $attribs=array(

            'data-image' => 'https://aionpowerbook.com/images/' . $ln . '.png',
