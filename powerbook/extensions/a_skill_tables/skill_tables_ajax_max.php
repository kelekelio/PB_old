...
if ($devanion_skill == 'custom') {
	...
	return '<img class="thumb" src="https://aionpowerbook.com/skillicon/' . $skilliconname . '.png" width="37" height="37" alt="">' . '|' . $skillrace . ' <a class="all skilltipzy" href="https://aionpowerbook.com/powerbook/Skill/' . $skillid . '"  skillid="' . $skillid . '" skilltiplang="' . $language . '" classic="' . isClassic() . '">' . $localization . '</a>'. $skilllevel . '|' . $stigma . '|' . $cost . '|' . $castingdelay .'|'. $delaytime;
	
	
	
}

function checkhigher($skill, $class)
{
	
$lastcharacter = substr($skill, -1);  // last character (level) of a skill, WI_Dark_SummonTornado_G2 => 2
$nextlevel = $lastcharacter + 1;
$likename = substr_replace($skill,"",-1) . $nextlevel;
...
$sql .= "SELECT id, class, skill ";
$sql .= "FROM  client_skill_learns ";
$sql .= "WHERE class = '$class' and skill = '$likename' ";
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	...
	$highestlevelonly = checkhigher($skill, $class);

	if ($highestlevelonly == '1') {
...
file_put_contents('../AionExtensions/cache/skilltables/'.$language.'/table_'.$class. $cl . '_max.json', $final);
